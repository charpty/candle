...
这次 bug hunt 累计修了 162 个 public API 边界问题，覆盖 Candle 的三十九条典型边界：
...
| `Tensor::chunk(0, dim)` | `candle-core` Tensor API | core API 的 `chunks` 也会参与除法；所有 divisor 参数都要先拒绝 0，再做 shape 计算。 |
| `Tensor::unfold` 零 step 和错误 op 名 | `candle-core` Tensor view API | view API 会直接构造 shape/stride；step 必须先拒绝 0，离散 shape 计算应使用整数公式，错误信息要指向真实 op。 |
| `Tensor::var` 空/单元素 reduction | `candle-core` Tensor reduction API | unbiased variance 的分母是 `n - 1`；统计定义里的 `n >= 2` 必须先写成代码合同，避免 `usize` 下溢和 NaN。 |
| `avg_pool2d` / `max_pool2d` 零 kernel/stride | `candle-core` Tensor pooling API | pooling 输出 shape 同时依赖 kernel、stride 和输入尺寸；窗口合同要在进入 backend 前统一闭合。 |
| `upsample_bilinear2d_with_scale` 非法 scale | `candle-core` Tensor upsample API | `f64` scale 进入 shape 计算前要检查有限正数，派生出的输出尺寸也要再次校验非零。 |
...
| `Tensor::slice_assign` range bound 溢出 | `candle-core` Tensor indexing API | `RangeBounds` 的 included/excluded 转换也会做 `+ 1`，同样需要 checked arithmetic。 |
...
| Qwen3-VL forward glue 输入 | `candle-transformers` glue 层 | `seqlens`、`seqlen_offsets` 和 image/video placeholder span 是跨模块协议，必须在调用入口校验。 |
| Qwen3-VL multimodal 元数据配对 | `candle-transformers` glue 层 | grid 元数据、placeholder span 和 pixel tensor 是同一份跨模块合同，缺少任一侧都不能静默退化成文本路径。 |
| Qwen3-VL vision grid 输入 | `candle-transformers` 多模态模型 | `grid_thw` 是动态 shape 元数据，列数、merge 整除关系、token 总数和溢出都要在插值/reshape 前验证。 |
| Stable Diffusion 构造配置 | `candle-transformers` diffusion 模型 | UNet/VAE 这类配置数组不能默认非空；public 构造函数要先校验 Vec 长度，再取 `[0]` 或 `.last()`。 |
| Stable Diffusion VAE latent 参数 | `candle-transformers` diffusion 模型 | `Tensor::chunk` 可能返回少于请求数量的块；拆 mean/logvar 前必须确认通道数是非零偶数。 |
| PaddleOCR-VL vision 配置 | `candle-transformers` 多模态模型 | 同类 NaViT/2D-RoPE 模型也要校验 heads、patch/image size、RoPE head dim 和 spatial merge。 |
| PaddleOCR-VL text 配置和空序列 | `candle-transformers` 多模态模型 | Text decoder 的 attention heads、KV heads、head_dim 和空序列输入同样要在构造/forward 入口闭合。 |
...
这 162 个 bug 值得放进学习资料，因为它们展示了 Rust/Candle 源码审计的正确顺序：
